...
- **절대 금지 사항 없음.** 과거 버전의 서버별 사용 금지 제약은 모두 해제되었다.
- 어떤 작업이든 가장 적합한 서버를 우선 사용하되, 필요하면 다른 서버의 도구로 폴백(fallback)해도 된다.
- 권장 라우팅은 효율성·일관성을 위한 가이드일 뿐, 강제 규칙이 아니다.

---

## TOOL ROUTING (권장 우선순위 — 강제 아님)

아래 표는 **각 작업에 가장 적합한 MCP**를 안내한다. "권장"은 우선 선택지를, "폴백 가능"은 풀 권한 하에 대안으로 사용 가능한 서버를 뜻한다.
...
- subtask description/상태 변경은 **eightton PATCH**를 우선 사용한다 — 한 번의 호출로 GitHub issue body가 자동 동기화되기 때문이다. (claude-code로 직접 편집하는 것도 허용되지만 동기화는 직접 처리해야 한다.)
- push·PR은 **eightton MCP API**를 우선 사용해 세션 기록과 일관성을 유지한다. (`gh` CLI / `git push` 폴백도 허용된다.)
...
이슈, subtask, 세션, Git push/PR을 **관리**하는 서버. 풀 권한을 가지며, 프로젝트 관리 작업에 우선 사용한다.
...
파일 편집, bash 실행, git 조작을 수행하는 서버. 풀 권한을 가지며, 코드 작업에 우선 사용한다. 필요 시 이슈/subtask 관리 작업의 폴백으로도 사용할 수 있다.
...
1. **도구 선택** — 모든 MCP는 풀 권한을 가진다. TOOL ROUTING 표의 권장 우선순위(subtask 관리 = eightton, 코드 작업 = claude-code, 테스트 = pitcrew)를 따르되, 필요하면 폴백 서버를 자유롭게 사용한다. 금지된 서버는 없다.
